...
{
	int volume = Fx_Volume( 255 );	// scale the volume by the current game config setting.
	SYS_TriggerSound3D( sound, point, volume );
}
...
void Play_Q_Samples( Orientation *o ) {
	...
	SYS_SetListenerOrientation( o->position, o->front, o->up );
...
void Mix_Voice_Sample( SYS_SOUNDHANDLE sound ) 
{
	...
	SYS_TriggerSoundVolume( sound, (unsigned char)volume );
}
...
void Play_Voice( SYS_SOUNDHANDLE sound ) 
{
	...
	SYS_PlaySoundVolume( sound, 0, (unsigned char)volume );
